...
- Runtime status: partial PWA control plane; no Supabase project, inference or
  operational backend exists
- Review target: Spec 0005 and Architecture 0002 plus the future synthetic pilot
...
The repository implements configuration validation, verified-claims session
protection, no-mock empty states and foreground camera cleanup. RLS, signed nodes,
inference, evidence, retention and operational controls do not exist yet; they remain
release requirements rather than current mitigations.
...
- implemented Next.js PWA camera/command shell, planned application API, Supabase
  control plane, Vercel deployment, edge inference, signed packages/events,
  watchlists, evidence, review, audit, and disabled public projection described in
  Architecture 0002 and Specs 0002–0005;
...
- mobile and desktop use one foreground-camera Next.js PWA;
...
- **Implemented PWA edge shell:** explicit browser camera lifecycle with a local
  `MediaStream`; device key, signed packages, inference, outbox and health are planned.
  Evidence: Architecture 0002, Spec 0005 and `apps/web`.
- **Implemented command-center shell / planned API:** Next.js UI plus verified-claims
  session protection; deployment,
...
- Human user → Supabase Auth: email/password, recovery, refresh tokens, and TOTP cross
  ...
  actions. Cookie refresh and verified-claims protection are implemented conditionally;
  MFA, membership and RLS await Supabase. Evidence: ADR-0008, Spec 0005 and
  `apps/web/proxy.ts`.
...
- Camera → PWA edge pipeline: untrusted frames cross the browser media boundary.
  Explicit permission and track cleanup are implemented. Capture masks, bounded
  buffers, parser/resource limits and local minimization remain planned; models are not
  selected or sandboxed. Evidence: Architecture 0002 and Spec 0005.
